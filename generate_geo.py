#!/usr/bin/env python3
"""
Bylder.com — GEO Content Generator
Genereert alle pagina's uit de GEO-strategie
"""
import json, re, sys
from pathlib import Path
from datetime import date
sys.path.insert(0, str(Path(__file__).parent))
from geo_content_data import GEO_PAGINAS
...
NAV = '''<nav style="position:fixed;top:0;left:0;right:0;z-index:50;background:rgba(245,240,232,0.92);backdrop-filter:blur(20px);border-bottom:1px solid rgba(61,46,30,0.08);padding:14px 0;">
  <div style="max-width:1280px;margin:0 auto;padding:0 48px;display:flex;align-items:center;justify-content:space-between;">
    ...
    <div style="display:flex;align-items:center;gap:24px;">
      <a href="/nieuwbouw-gids/" style="font-size:14px;color:rgba(61,46,30,0.6);text-decoration:none;">Gidsen</a>
...
body{background:#F5F0E8;color:#3D2E1E;font-family:'Plus Jakarta Sans',sans-serif;line-height:1.6}
.container{max-width:1280px;margin:0 auto;padding:0 48px}
@media(max-width:768px){.container{padding:0 20px}.sidebar{display:none!important}}
.warm-divider{background:linear-gradient(90deg,transparent,rgba(200,184,154,0.5),transparent);height:1px}
table{width:100%;border-collapse:collapse;margin:20px 0;font-size:14px}
th{background:#1A1208;color:#F5F0E8;padding:12px 14px;text-align:left;font-weight:700;font-size:12px;text-transform:uppercase;letter-spacing:0.05em}
td{padding:11px 14px;border-bottom:1px solid rgba(61,46,30,0.08);color:rgba(61,46,30,0.75);vertical-align:top}
tr:nth-child(even) td{background:rgba(245,240,232,0.5)}
tr:last-child td{font-weight:700;color:#1A1208;background:rgba(61,90,62,0.06)}
.fb{max-height:0;overflow:hidden;transition:max-height 0.35s ease}
.fb.o{max-height:600px!important}
.badge-geo{background:rgba(201,168,76,0.12);border:1px solid rgba(201,168,76,0.25);color:#A88030;display:inline-flex;align-items:center;gap:6px;padding:4px 12px;border-radius:999px;font-size:11px;font-family:'Space Mono',monospace;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:14px}
.source-box{background:rgba(61,46,30,0.04);border-left:3px solid rgba(61,46,30,0.15);padding:12px 16px;border-radius:0 8px 8px 0;font-size:12px;color:rgba(61,46,30,0.5);font-family:'Space Mono',monospace;margin-top:32px;line-height:1.6}
</style>'''

def render_tabel(tabel):
    if not tabel:
        return ""
    h = "".join(f"<th>{c}</th>" for c in tabel["headers"])
    rows = ""
    for r in tabel["rijen"]:
        cells = "".join(f"<td>{c}</td>" for c in r)
        rows += f"<tr>{cells}</tr>"
    return f'''<div style="overflow-x:auto;margin:20px 0;border-radius:12px;overflow:hidden;border:1px solid rgba(61,46,30,0.1);">
<table><thead><tr>{h}</tr></thead><tbody>{rows}</tbody></table></div>'''

def render_secties(secties):
    html = ""
    for s in secties:
        html += f'<h2 style="font-size:1.35rem;font-weight:800;color:#1A1208;letter-spacing:-0.02em;margin:36px 0 12px;">{s["h2"]}</h2>'
        if s.get("tabel"):
            html += render_tabel(s["tabel"])
        if s.get("tekst"):
            html += f'<p style="font-size:16px;color:rgba(61,46,30,0.7);line-height:1.85;margin-bottom:8px;">{s["tekst"]}</p>'
        for h3 in s.get("h3s", []):
            html += f'<h3 style="font-size:1.1rem;font-weight:700;color:#1A1208;margin:20px 0 8px;">{h3["h3"]}</h3>'
            html += f'<p style="font-size:15px;color:rgba(61,46,30,0.7);line-height:1.8;margin-bottom:8px;">{h3["tekst"]}</p>'
    return html

def render_faq(faq_items, fc="#3D5A3E"):
    if not faq_items:
        return ""
    items = ""
    for f in faq_items:
        items += f'''<div style="border:1px solid rgba(61,46,30,0.09);border-radius:14px;overflow:hidden;background:#fff;margin-bottom:10px;">
...
<div class="fb"><div style="padding:0 22px 18px;font-size:14px;line-height:1.75;color:rgba(61,46,30,0.7);">{f["antwoord"]}</div></div>
</div>'''
    return f'<h2 style="font-size:1.35rem;font-weight:800;color:#1A1208;letter-spacing:-0.02em;margin:40px 0 16px;">Veelgestelde vragen</h2>{items}'

def render_page(p):
    sectie = p["sectie"]
    slug   = p["slug"]
    url    = f"{BASE_URL}/{sectie}/{slug}/"
    ptype  = p["type"]

    # Type badges
    badge_map = {
        "snippet":   ("AI-snippet · Meest geciteerd", "#A88030"),
        "deep-dive": ("Deep-dive · Uitgebreid onderzoek", "#3D5A3E"),
        "qa":        ("Vraag & Antwoord · AI-geoptimaliseerd", "#B85C38"),
    }
    badge_label, badge_kleur = badge_map.get(ptype, ("Gids", "#3D5A3E"))

    # Schema.org
    faq_ld = None
    if p.get("faq"):
        faq_ld = {"@context":"https://schema.org","@type":"FAQPage","mainEntity":[
            {"@type":"Question","name":f["vraag"],"acceptedAnswer":{"@type":"Answer","text":f["antwoord"]}}
            for f in p["faq"]
        ]}

    # Article LD
    art_ld = {"@context":"https://schema.org","@type":"Article","headline":p["h1"],
              "description":p["meta_desc"],"url":url,"datePublished":TODAY,"dateModified":TODAY,
              "publisher":{"@type":"Organization","name":"Bylder.com","url":BASE_URL}}

    # Breadcrumb
    fase_naam = sectie.split("/")[-1].replace("-"," ").title() if "/" in sectie else sectie
    bc_ld = {"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[
        {"@type":"ListItem","position":1,"name":"Bylder","item":BASE_URL},
        {"@type":"ListItem","position":2,"name":"Gidsen","item":f"{BASE_URL}/nieuwbouw-gids/"},
        {"@type":"ListItem","position":3,"name":fase_naam,"item":f"{BASE_URL}/{sectie}/"},
        {"@type":"ListItem","position":4,"name":p["h1"],"item":url},
    ]}

    ld_blocks = f'<script type="application/ld+json">{json.dumps(art_ld,ensure_ascii=False)}</script>\n'
    ld_blocks += f'<script type="application/ld+json">{json.dumps(bc_ld,ensure_ascii=False)}</script>\n'
    if faq_ld:
        ld_blocks += f'<script type="application/ld+json">{json.dumps(faq_ld,ensure_ascii=False)}</script>\n'

    # Tabel 1 en 2 (voor snippet type)
    tabel1_html = render_tabel(p.get("tabel_1")) if p.get("tabel_1") else ""
    tabel2_html = render_tabel(p.get("tabel_2")) if p.get("tabel_2") else ""

    # Secties
    secties_html = render_secties(p.get("secties", []))
    ...
    # Breadcrumb parts
    bc_parts = sectie.split("/")
    bc_html = f'<a href="/" style="color:rgba(61,46,30,0.4);text-decoration:none;">Bylder</a> › <a href="/nieuwbouw-gids/" style="color:rgba(61,46,30,0.4);text-decoration:none;">Gidsen</a>'
    if len(bc_parts) > 1:
        bc_html += f' › <a href="/{sectie}/" style="color:rgba(61,46,30,0.4);text-decoration:none;">{fase_naam}</a>'
    bc_html += f' › <span style="color:{badge_kleur};font-weight:600;">{p["slug"].replace("-"," ").title()[:40]}</span>'
...
<nav style="display:flex;align-items:center;gap:6px;font-size:12px;color:rgba(61,46,30,0.4);flex-wrap:wrap;">{bc_html}</nav>
</div></div>
...
<section style="padding:40px 0 48px;background:#fff;">
  <div class="container" style="max-width:820px;">
    <div class="badge-geo" style="background:rgba({",".join(str(int(badge_kleur.lstrip("#")[i:i+2],16)) for i in (0,2,4))},0.1);border-color:rgba({",".join(str(int(badge_kleur.lstrip("#")[i:i+2],16)) for i in (0,2,4))},0.25);color:{badge_kleur};">{badge_label}</div>
...
<section style="padding:48px 0;">
  <div style="max-width:1280px;margin:0 auto;padding:0 48px;display:grid;grid-template-columns:1fr 300px;gap:48px;align-items:start;">
    <article>
      {tabel1_html}
      {tabel2_html}
      {secties_html}
    ...
    <aside class="sidebar" style="position:sticky;top:80px;">
      ...
      <div style="background:#fff;border:1px solid rgba(61,46,30,0.08);border-radius:14px;padding:18px;">
        <div style="font-size:11px;font-family:'Space Mono',monospace;text-transform:uppercase;letter-spacing:0.08em;color:rgba(61,46,30,0.35);margin-bottom:10px;">Gerelateerde gidsen</div>
        <div style="display:flex;flex-direction:column;gap:7px;">
          ...
          <a href="/tools/meerwerk-calculator/" style="font-size:13px;color:rgba(61,46,30,0.6);text-decoration:none;">Meerwerk calculator</a>
          <a href="/tools/5-procent-depot-calculator/" style="font-size:13px;color:rgba(61,46,30,0.6);text-decoration:none;">5% depot calculator</a>
...
# ── GENEREER ALLES ──────────────────────────────────────────────────────────
generated = 0
for p in GEO_PAGINAS:
    out_dir = OUTPUT / p["sectie"] / p["slug"]
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "index.html").write_text(render_page(p), encoding="utf-8")
    url = f"{BASE_URL}/{p['sectie']}/{p['slug']}/"
    generated += 1
    print(f"[OK] {url}")

print(f"\n{'='*55}")
print(f"Gegenereerd: {generated} GEO-pagina's")
print(f"{'='*55}")
