...
const DISCLAIMER =
  'Prijzen zijn indicatieve marktbandbreedtes (NL 2026) en verschillen per project, regio en afwerking. Reviewscores zijn afkomstig van de genoemde externe platforms; bekijk de volledige beoordelingen bij de bron. Bylder is zelf geen badkamerbedrijf.'
...
function getBedrijfHtml(page: BadkamerPage): string {
  ...
  const extra = extraLinksHtml(CLUSTER, page.slug, 'badkamerspecialisten')
  return metEersteZet(
    metVerleider(body.replace('</main>', `${extra}${markt}${claim}${DISCLAIMER_HTML}</main>`), 'badkamer'),
    esc(b.name))
}
...
function getMainHtmlRuw(page: BadkamerPage): string {
