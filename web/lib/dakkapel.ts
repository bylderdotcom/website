// Fase 2 — city+bedrijf-cluster /dakkapel/ (1.700 pagina's, 1.214 geïndexeerd).
// LAATSTE van de 8 city+bedrijf-clusters — na dit cluster is die hele laag
// klaar. Clone van web/lib/gietvloer.ts. Simpelste vorm (zoals badkamer): geen
// city_alt/name_alt, alle bedrijf-entries hebben city + maps_href, kaart-vormen
// alleen rated/unrated. Leest de canonieke datalaag op build-time; alleen de
...
const DISCLAIMER =
  'Prijzen zijn indicatieve marktbandbreedtes (NL 2026) en verschillen per project, regio en afwerking. Reviewscores zijn afkomstig van de genoemde externe platforms; bekijk de volledige beoordelingen bij de bron. Bylder is zelf geen dakkapelbedrijf.'
...
let _pages: DakkapelPage[] | null = null
export function getPages(): DakkapelPage[] {
  if (!_pages) {
    const real: DakkapelPage[] = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'pages.json'), 'utf8'))
    _pages = [...real, ...buildRegisterPages()]
  }
  return _pages!
}
export function getPage(slug: string): DakkapelPage | undefined {
...
function buildRegisterPages(): DakkapelPage[] {
...
function getRegisterHtml(page: DakkapelPage): string {
...
// Eén uniforme shell voor alle 1.700 pagina's (pages.json "template"-veld is
// overal "default").
...
function getCityHtml(page: DakkapelPage): string {
...
function claimHtml(pageSlug: string, naam: string): string {
  // De slug in de URL mist de vakprefix — die staat al in het pad
    // (/dakkapel/bedrijf/...), dus de generator haalt hem uit de slug. In de
    // database heet hetzelfde record wél 'dakkapel-<slug>'. De claimpagina zocht op
  ...
  const slug = 'dakkapel-' + pageSlug.replace(/^bedrijf\//, '')   // vakprefix
  return '<div class="divider"></div>'
...
function getBedrijfHtml(page: DakkapelPage): string {
  ...
  return metVerleider(body.replace('</main>', `${markt}${claim}${DISCLAIMER_HTML}</main>`), 'dakkapel')
}
...
function getMainHtmlRuw(page: DakkapelPage): string {
...
export function toMetadata(page: DakkapelPage): Metadata {
