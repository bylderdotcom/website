...
const DISCLAIMER =
  'Prijzen zijn indicatieve marktbandbreedtes (NL 2026) en verschillen per project, regio en afwerking. Reviewscores zijn afkomstig van de genoemde externe platforms; bekijk de volledige beoordelingen bij de bron. Bylder is zelf geen schildersbedrijf.'
...
function claimHtml(pageSlug: string, naam: string): string {
  // De slug in de URL mist de vakprefix — die staat al in het pad
    // (/schilder/bedrijf/...), dus de generator haalt hem uit de slug. In de
    // database heet hetzelfde record wél 'schilder-<slug>'. De claimpagina zocht op
  ...
  const slug = 'schilder-' + pageSlug.replace(/^bedrijf\//, '')   // vakprefix
  return '<div class="divider"></div>'
