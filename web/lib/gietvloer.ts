// Fase 2 — eerste city+bedrijf-cluster /gietvloer/ (657 pagina's, 479 geïndexeerd).
// Nieuwe render-vorm t.o.v. de vakstad-clusters (project/kopen): stadspagina's
// tonen een grid van bedrijfskaarten (uit cities.json), en elk bedrijf heeft een
// eigen los-geïndexeerd profiel (bedrijven.json) met rating/contact-rijen en
// "andere bedrijven"-tegels. Blauwdruk voor de overige 7 city+bedrijf-clusters
// (loodgieter/aannemer/schilder/elektricien/badkamer/stukadoor/dakkapel).
...
// Disclaimer uit de cluster-footer (templates/clusters/gietvloer/footer.default.html):
// unieke E-E-A-T-tekst, niet aanwezig in de city/bedrijf-contentfragmenten (de
...
let _pages: GietvloerPage[] | null = null
export function getPages(): GietvloerPage[] {
  if (!_pages) {
    const real: GietvloerPage[] = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'pages.json'), 'utf8'))
    _pages = [...real, ...buildRegisterPages()]
  }
  return _pages!
}
export function getPage(slug: string): GietvloerPage | undefined {
...
function buildRegisterPages(): GietvloerPage[] {
...
function getRegisterHtml(page: GietvloerPage): string {
...
// Eén uniforme shell voor alle 657 pagina's (pages.json "template"-veld is
// overal "default"), anders dan project/kopen waar de shell per pagina wisselt.
...
function getCityHtml(page: GietvloerPage): string {
...
function claimHtml(pageSlug: string, naam: string): string {
  // De slug in de URL mist de vakprefix — die staat al in het pad
    // (/gietvloer/bedrijf/...), dus de generator haalt hem uit de slug. In de
    // database heet hetzelfde record wél 'gietvloer-<slug>'. De claimpagina zocht op
  ...
  const slug = 'gietvloer-' + pageSlug.replace(/^bedrijf\//, '')   // vakprefix
  return '<div class="divider"></div>'
...
function getBedrijfHtml(page: GietvloerPage): string {
...
function getMainHtmlRuw(page: GietvloerPage): string {
...
export function toMetadata(page: GietvloerPage): Metadata {
...
export function getMainHtml(page: GietvloerPage): string {
