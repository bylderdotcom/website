// Fase 2 — city+bedrijf-cluster /stukadoor/ (3.389 pagina's, 1.499 geïndexeerd).
// Clone van web/lib/gietvloer.ts. Verschillen: extra card-vorm v3 (zoals
// loodgieter/elektricien, geen Maps-link); één bedrijf-entry mist city/
// city_slug (r-seuren-stucadoorsbedrijf, template v16 — bevat die placeholders
// dan ook niet). Leest de canonieke datalaag op build-time; alleen de
// renderlaag migreert. Server/build-only (fs).
...
// Disclaimer uit de cluster-footer (templates/clusters/stukadoor/footer.default.html):
...
const DISCLAIMER =
  'Prijzen zijn indicatieve marktbandbreedtes (NL 2026) en verschillen per project, regio en afwerking. Reviewscores zijn afkomstig van de genoemde externe platforms; bekijk de volledige beoordelingen bij de bron. Bylder is zelf geen stukadoorsbedrijf.'
...
let _pages: StukadoorPage[] | null = null
export function getPages(): StukadoorPage[] {
  if (!_pages) {
    const real: StukadoorPage[] = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'pages.json'), 'utf8'))
    _pages = [...real, ...buildRegisterPages()]
  }
  return _pages!
}
export function getPage(slug: string): StukadoorPage | undefined {
...
function buildRegisterPages(): StukadoorPage[] {
...
function getRegisterHtml(page: StukadoorPage): string {
...
// Eén uniforme shell voor alle 3.389 pagina's (pages.json "template"-veld is
// overal "default").
...
function getCityHtml(page: StukadoorPage): string {
...
function claimHtml(pageSlug: string, naam: string): string {
  // De slug in de URL mist de vakprefix — die staat al in het pad
    // (/stukadoor/bedrijf/...), dus de generator haalt hem uit de slug. In de
    // database heet hetzelfde record wél 'stukadoor-<slug>'. De claimpagina zocht op
  ...
  const slug = 'stukadoor-' + pageSlug.replace(/^bedrijf\//, '')   // vakprefix
  return '<div class="divider"></div>'
...
function getBedrijfHtml(page: StukadoorPage): string {
  ...
  const extra = extraLinksHtml(CLUSTER, page.slug, 'stukadoors')
  return metEersteZet(
    metVerleider(body.replace('</main>', `${extra}${markt}${claim}${DISCLAIMER_HTML}</main>`), 'stukadoor'),
    esc(b.name))
}
...
function getMainHtmlRuw(page: StukadoorPage): string {
...
export function toMetadata(page: StukadoorPage): Metadata {
...
export function getMainHtml(page: StukadoorPage): string {
