// Fase 2 — city+bedrijf-cluster /elektricien/ (3.886 pagina's, 2.428 geïndexeerd).
// Clone van web/lib/gietvloer.ts. Verschillen: extra card-vorm v3 (zoals
// loodgieter, geen Maps-link) naast rated/unrated; 12 bedrijf-entries missen
// city/city_slug (templates v12/v13); veel entries missen maps_href zonder
// maps_cid_href-alternatief (contact_row-variant zonder Maps-link, bv. v5) —
// allemaal gedekt door de generieke "vul alleen in wat aanwezig is"-aanpak,
// geen speciale code nodig. Leest de canonieke datalaag op build-time; alleen
// de renderlaag migreert. Server/build-only (fs).
...
// Disclaimer uit de cluster-footer (templates/clusters/elektricien/footer.default.html):
...
const DISCLAIMER =
  'Prijzen zijn indicatieve marktbandbreedtes (NL 2026) en verschillen per project, regio en afwerking. Reviewscores zijn afkomstig van de genoemde externe platforms; bekijk de volledige beoordelingen bij de bron. Bylder is zelf geen elektriciensbedrijf.'
...
let _pages: ElektricienPage[] | null = null
export function getPages(): ElektricienPage[] {
  if (!_pages) {
    const real: ElektricienPage[] = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'pages.json'), 'utf8'))
    _pages = [...real, ...buildRegisterPages()]
  }
  return _pages!
}
export function getPage(slug: string): ElektricienPage | undefined {
...
function buildRegisterPages(): ElektricienPage[] {
...
function getRegisterHtml(page: ElektricienPage): string {
...
// Eén uniforme shell voor alle 3.886 pagina's (pages.json "template"-veld is
// overal "default").
...
function getCityHtml(page: ElektricienPage): string {
...
function claimHtml(pageSlug: string, naam: string): string {
  // De slug in de URL mist de vakprefix — die staat al in het pad
    // (/elektricien/bedrijf/...), dus de generator haalt hem uit de slug. In de
    // database heet hetzelfde record wél 'elektricien-<slug>'. De claimpagina zocht op
  ...
  const slug = 'elektricien-' + pageSlug.replace(/^bedrijf\//, '')   // vakprefix
  return '<div class="divider"></div>'
    ...
    + 'Dit is jouw bedrijf? Neem het gratis over.</h2>'
    + '<p style="font-size:14px;color:rgba(61,46,30,0.74);line-height:1.7;margin:0 0 10px;">'
    + 'Dit profiel is samengesteld uit openbare bronnen, zonder dat je erom vroeg. Je kunt het '
    + 'overnemen en er je eigen omschrijving, diensten en werkgebied in zetten &mdash; en je ziet '
    + 'dan hoe vaak het getoond en aangeklikt is. <strong>Daar zit geen prijskaartje aan.</strong></p>'
    + '<p style="font-size:14px;color:rgba(61,46,30,0.74);line-height:1.7;margin:0 0 12px;">'
    + 'We controleren eerst even of het echt van jou is: staat je e-mailadres op het domein van je '
    + 'website, dan krijg je meteen een code. Anders bellen we het nummer dat op dit profiel staat.</p>'
    ...
    + 'Claim dit profiel &mdash; gratis &#8594;</a>'
    + '<p style="font-size:13.5px;color:rgba(61,46,30,0.66);line-height:1.7;margin:14px 0 0;">'
    + 'Wil je daarna ook reageren op aanvragen uit je eigen plaats, inkoopvoordeel, en een '
    + 'vergoeding vanaf 1% op wat je klant via Bylder koopt? Dat is het partnerschap, '
    + '&euro;79 per jaar &mdash; bij een aankoop van &euro;4.000 is 1% al &euro;40.</p>'
    ...
    + '<a href="/voor-vakbedrijven/" style="color:#3D5A3E;">wat je verder krijgt</a> &middot; '
    + '<a href="https://app.bylder.com/profiel-verwijderen?slug=' + encodeURIComponent(slug)
    + '" rel="nofollow" style="color:rgba(61,46,30,0.55);">liever helemaal niet op Bylder</a></p>'
    + '</div>'
}

function getBedrijfHtml(page: ElektricienPage): string {
  ...
  const extra = extraLinksHtml(CLUSTER, page.slug, 'elektriciens')
  return metEersteZet(
    metVerleider(body.replace('</main>', `${extra}${markt}${claim}${DISCLAIMER_HTML}</main>`), 'elektricien'),
    esc(b.name))
}
...
function getMainHtmlRuw(page: ElektricienPage): string {
...
export function toMetadata(page: ElektricienPage): Metadata {
...
export function getMainHtml(page: ElektricienPage): string {
