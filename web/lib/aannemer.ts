// Fase 2 — city+bedrijf-cluster /aannemer/ (4.717 pagina's, 2.682 geïndexeerd).
// Clone van web/lib/gietvloer.ts. Verschillen: gebruikt overal {{maps_href}}
// (zoals gietvloer, niet {{maps_cid_href}} zoals loodgieter); kaart-vormen zijn
// alleen rated/unrated (geen extra v3/v4). Eén bedrijf-entry mist city/city_slug
// (bedrijf/bouwbedrijf-van-der-werff-8zq6d8, template v15) — die contentvariant
// bevat {{city}}/{{city_slug}} dan ook niet, dus de generieke "vul alleen in wat
// aanwezig is"-aanpak dekt dit al. Leest de canonieke datalaag op build-time;
// alleen de renderlaag migreert. Server/build-only (fs).
...
// Disclaimer uit de cluster-footer (templates/clusters/aannemer/footer.default.html):
...
const DISCLAIMER =
  'Prijzen zijn indicatieve marktbandbreedtes (NL 2026) en verschillen per project, regio en afwerking. Reviewscores zijn afkomstig van de genoemde externe platforms; bekijk de volledige beoordelingen bij de bron. Bylder is zelf geen aannemersbedrijf.'
...
let _pages: AannemerPage[] | null = null
export function getPages(): AannemerPage[] {
  if (!_pages) {
    const real: AannemerPage[] = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'pages.json'), 'utf8'))
    _pages = [...real, ...buildRegisterPages()]
  }
  return _pages!
}
export function getPage(slug: string): AannemerPage | undefined {
...
function buildRegisterPages(): AannemerPage[] {
...
function getRegisterHtml(page: AannemerPage): string {
...
// Eén uniforme shell voor alle 4.717 pagina's (pages.json "template"-veld is
// overal "default").
...
function getCityHtml(page: AannemerPage): string {
...
function claimHtml(pageSlug: string, naam: string): string {
  // De slug in de URL mist de vakprefix — die staat al in het pad
    // (/aannemer/bedrijf/...), dus de generator haalt hem uit de slug. In de
    // database heet hetzelfde record wél 'aannemer-<slug>'. De claimpagina zocht op
  ...
  const slug = 'aannemer-' + pageSlug.replace(/^bedrijf\//, '')   // vakprefix
  return '<div class="divider"></div>'
...
function getBedrijfHtml(page: AannemerPage): string {
  ...
  const extra = extraLinksHtml(CLUSTER, page.slug, 'aannemers')
  return metEersteZet(
    metVerleider(body.replace('</main>', `${extra}${markt}${claim}${DISCLAIMER_HTML}</main>`), 'aannemer'),
    esc(b.name))
}
...
function getMainHtmlRuw(page: AannemerPage): string {
...
export function toMetadata(page: AannemerPage): Metadata {
...
export function getMainHtml(page: AannemerPage): string {
