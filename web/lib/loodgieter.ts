// Fase 2 — city+bedrijf-cluster /loodgieter/ (5.391 pagina's, 3.516 geïndexeerd).
// Clone van web/lib/gietvloer.ts (zelfde city+bedrijf-render-vorm). Verschillen:
// - loodgieter-kaarten gebruiken overal {{maps_cid_href}} (maps.google.com/?cid=…)
//   i.p.v. {{maps_href}} (google.com/maps/search) — beide velden worden generiek
//   doorgegeven, de templates gebruiken alleen wat ze nodig hebben.
// - 2 extra card-vormen (v3/v4) naast rated/unrated (v3 = geen Maps-link, v4 =
//   met prijsniveau-badge) — vandaar `shape: string` i.p.v. een vaste union.
...
type Company = {
  shape: string // 'rated' | 'unrated' | 'v3' | 'v4'
  profile_href: string
  name: string
  plaats: string
  rating: string
...
// Disclaimer uit de cluster-footer (templates/clusters/loodgieter/footer.default.html):
...
let _pages: LoodgieterPage[] | null = null
export function getPages(): LoodgieterPage[] {
  if (!_pages) {
    const real: LoodgieterPage[] = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'pages.json'), 'utf8'))
    _pages = [...real, ...buildRegisterPages()]
  }
  return _pages!
}
export function getPage(slug: string): LoodgieterPage | undefined {
...
function buildRegisterPages(): LoodgieterPage[] {
...
function getRegisterHtml(page: LoodgieterPage): string {
...
// Eén uniforme shell voor alle 5.391 pagina's (pages.json "template"-veld is
// overal "default").
...
function getCityHtml(page: LoodgieterPage): string {
...
function claimHtml(pageSlug: string, naam: string): string {
  // De slug in de URL mist de vakprefix — die staat al in het pad
    // (/loodgieter/bedrijf/...), dus de generator haalt hem uit de slug. In de
    // database heet hetzelfde record wél 'loodgieter-<slug>'. De claimpagina zocht op
  ...
  const slug = 'loodgieter-' + pageSlug.replace(/^bedrijf\//, '')   // vakprefix
  return '<div class="divider"></div>'
...
function getBedrijfHtml(page: LoodgieterPage): string {
  ...
  if (b.rating_row) {
    const row = fillPlaceholders(readTpl(`row.rating_row.${b.rating_row}.html`), {
      rating_disp: b.rating_disp, reviews: b.reviews, maps_href: b.maps_href, maps_cid_href: b.maps_cid_href,
    })
    body = body.replace('{{rating_row}}', row)
  }
  if (b.contact_row) {
    const row = fillPlaceholders(readTpl(`row.contact_row.${b.contact_row}.html`), {
      website: b.website, tel: b.tel, tel_disp: b.tel_disp, maps_href: b.maps_href, maps_cid_href: b.maps_cid_href,
    })
  ...
  const extra = extraLinksHtml(CLUSTER, page.slug, 'loodgieters')
  return metEersteZet(
    metVerleider(body.replace('</main>', `${extra}${markt}${claim}${DISCLAIMER_HTML}</main>`), 'loodgieter'),
    esc(b.name))
}
...
function getMainHtmlRuw(page: LoodgieterPage): string {
...
export function toMetadata(page: LoodgieterPage): Metadata {
...
export function getMainHtml(page: LoodgieterPage): string {
